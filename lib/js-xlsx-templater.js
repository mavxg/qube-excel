...
Workbook.prototype.read_master_sheet = function(){
	...
	return sheet.match(/name=\"([\w\s]*)\"\ssheetId=\"[0-9]*\"/g).map(function(d){
		return { name:d.match(/name=\"([\w\s]*)\"/)[1], 
		         id:d.match(/sheetId=\"(\w*)\"/)[1]}
	});
};
...
Workbook.prototype.find_table_path = function(name){
	var tab = this.read_sheet(name);
	var tableXml = tab.match(/[a-z0-9]*\.xml/g)[0];
	return 'xl/tables/'+tableXml;	
};
...
Workbook.prototype.clean_cache = function(){
	...
	wb.sheet_names().forEach(function(sheet){
		...
		if(cells){
			...
			if(changed) wb.zip.files[alias]._data = utils.stringToUint8Array(data);
		}
	});
}
...
Workbook.prototype.replace_sheet_data = function(sheetName, data, range){
	...
	sheet = sheet.replace(/\<dimension ref="[\w\d:]*"\/\>/g, range); //Replace the range
...
Workbook.prototype.replace_sheet_contents = function(sheetName, dataSheet){
	var alias = this.find_sheet_path(sheetName);
	this.zip.files[alias]._data = utils.stringToUint8Array(dataSheet);
}
