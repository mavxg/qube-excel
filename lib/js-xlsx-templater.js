...
Workbook.prototype.clean_cache = function(sheetNames){
	...
	sheetNames.forEach(function(sheet){
		...
		try {
			var trimmed = data.replace(/(\<c[^\>\<]+\>\<f[^\>\<]*\>[^\>\<]+\<\/f\>)(\<v\>[^\>\<]+\<\/v\>)/gi, "$1<v/>");
			if(data !== trimmed)
...
Workbook.prototype.set_calc_properties = function(props){
	var master = this.read_sheet('xl/workbook.xml');
	var newMstr = master.replace(/(\<calcPr\scalcId\=\"[0-9]*\")([^\>]+)(\/\>)/i,"$1 " + props + "$3");
	this.zip.files['xl/workbook.xml']._data = utils.stringToUint8Array(newMstr);
}
